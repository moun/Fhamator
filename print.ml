...
(*  | Assert t -> sprintf "assert (%s)"  (print_test tab t)
  ...
  | Inputh (l, lvars) -> sprintf "%s%sinputh%s%s" (f idt l) idt 
    (List.fold_left (fun accu elt -> sprintf "%s %s" accu (tab elt) ) "" lvars)
    end_block 
  | Inputl (l, lvars) -> sprintf "%s%sinputl%s%s" (f idt l) idt 
    (List.fold_left (fun accu elt -> sprintf "%s %s" accu (tab elt) ) "" lvars)
    end_block (* TODO *)
